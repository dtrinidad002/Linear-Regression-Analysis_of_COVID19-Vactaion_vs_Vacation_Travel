...
X1: vaccination rates
X2: Population testing positive for COVID-19
X3: COVID-19 related deaths
X4: COVID-19 testing
Y: community mobility levels

Our hypothesis is that the relationship between X1 and X4 will be positively correlated with Y, and X2 and X3 will be negatively correlated with Y.

Data Sources: 
[COVID-19 Community Mobility Report](https://zoom.us/j/94880650987) 
[Bureau of Transportation] (https://data.bts.gov/Research-and-Statistics/Trips-by-Distance/w96p-f2qv) 
[CDC Data on Vaccinations](https://data.cdc.gov/Vaccinations/COVID-19-Vaccinations-in-the-United-States-County/8xkx-amqh) More description of this data is available[here](https://covid.cdc.gov/covid-data-tracker/#vaccinations-county-view)
...
We will be cleaning and investigating the data in R and extracting the columns of note, as well as mutating new columns for needed variables.
Namely:
Clean and link the data sources so that they are in the same time frame, for the same state’s population by county.
Analyze the datasets for use in CLT statistics. (Size, Variance)
Describe the type of variables in the data sets and find the most appropriate ones for our research question.
Levene Test.
Define if properties for linear regression can be applied in our dataframe/variables.
Do linear regression of data
Analyze results 
Present conclusion.
...
patients <- read_csv('./src/data/covid19cases_test.csv')
vaccinations <- read_csv('./src/data/COVID-19_Vaccinations_in_the_United_States_County.csv')
mobility2021 <- read_csv('./src/data/2021_US_Region_Mobility_Report.csv')
```
...
relevant_vaccinations <- filter(relevant_vaccinations, Recip_State == 'CA', Date == '07/26/2021')
...
relevant_mobility <- select(mobility2021,
                            ...
                            contains("baseline"))
relevant_mobility <- filter(relevant_mobility, date == '2021-07-26')
relevant_mobility <- filter(relevant_mobility,country_region_code == 'US',sub_region_1 == 'California')

clean_mobility <- select(relevant_mobility,
                         sub_region_2,
                         contains("baseline"))
clean_mobility <-
    group_by(clean_mobility,sub_region_2) %>% 
    summarise(across(
      .cols = is.numeric,
...
names(PatMobVacc)[4] <-"Pct_Pop_Vacc_Complete"
```

```{r EDA, echo=FALSE, warning=TRUE, message=FALSE}

mob <- ggplot(data=clean_mobility, aes(x=Mean_Mobility))+
  geom_histogram()
  
cumu_death <- ggplot(data=clean_patients, aes(x=cumulative_deaths))+
  geom_histogram()
  mob | cumu_death

tot_test <-ggplot(data=clean_patients, aes(x=cumulative_total_tests))+
  geom_histogram()
  
cumu_p_test <-ggplot(data=clean_patients, aes(x=cumulative_positive_tests))+
  geom_histogram()
  tot_test|cumu_p_test

ggplot(data = clean_vaccinations, aes(x=Series_Complete_Pop_Pct))+
  geom_histogram()

d2<-PatMobVacc[,c("Mean_Mobility","Pct_Pop_Vacc_Complete","cumulative_deaths","cumulative_positive_tests", "cumulative_total_tests")]
pairs(d2)
...
ggplot(data = PatMobVacc, aes(x=Mean_Mobility, y= Pct_Pop_Vacc_Complete))+
  geom_point()+
  geom_smooth(method = lm)

model_short <- lm(Mean_Mobility ~  Pct_Pop_Vacc_Complete, data = PatMobVacc)

stargazer(
    model_short, 
    type = 'text', 
    se = list(get_robust_se(model_short))
  )
```



```{r Assumptions of Linearity}
plot(model_short, which = 1) #Linear Conditional Expectation

plot(model_short, which = 3) #Homoskedastic Errors

plot(model_short, which = 2) #Normally Distributed Errors

hist(model_short$residuals, breaks = 20, main = "Residuals from Linear Model Predicting Views")
#plot(model_short, which = 4)
#plot(model_short, which = 5)
#plot(model_short, which = 6)
```
...
g_log_CD <- ggplot(data=PatMobVacc, aes(x=log_cumulative_deaths))+
  geom_histogram()
  
g_log_CTT <- ggplot(data=PatMobVacc, aes(x=log_cumulative_total_tests))+
  geom_histogram()

g_log_CPT <- ggplot(data=PatMobVacc, aes(x=log_cumulative_positive_tests))+
  geom_histogram()
  g_log_CD | g_log_CPT | g_log_CTT

model <- lm(Mean_Mobility ~  Pct_Pop_Vacc_Complete + log_cumulative_deaths + log_cumulative_total_tests + log_cumulative_positive_tests, data = PatMobVacc)
...
```{r IDD and No Perfect Colinearity Variance Inflation Factor}
stargazer(vif(model), type = "text")
stargazer(vif(model)>4, type = "text")

```

```{r Assumptions of Linearity Long Model}
plot(model, which = 1) #Linear Conditional Expectation

plot(model, which = 3) #Homoskedastic Errors

plot(model, which = 2) #Normally Distributed Errors

hist(model$residuals, breaks = 20, main = "Residuals from Linear Model Predicting Views")
#plot(model, which = 4)
#plot(model, which = 5)
#plot(model, which = 6)
```
